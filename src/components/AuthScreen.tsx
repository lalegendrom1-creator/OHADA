...
export default function AuthScreen() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-stretch relative overflow-hidden">
      ...
      {/* ── LEFT PANEL: App info (desktop only) ── */}
      ...
      <div className="flex-1 flex flex-col relative overflow-y-auto">
        ...
        {/* ── MOBILE INFO SECTION (hidden on lg+) ── */}
        <div className="lg:hidden px-4 pt-10 pb-6 space-y-6">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm leading-relaxed mt-3">
              Accédez à une bibliothèque de modèles d'actes juridiques validés,
              adaptés aux législations des 17 pays de l'espace OHADA.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {STATS.map(({ value, label, icon: Icon }) => (
              <div key={label} className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-3 text-center">
                <Icon size={15} className="text-amber-400 mx-auto mb-1.5" />
          ...
          <div>
            ...
            <div className="space-y-2">
              {CATEGORIES.map(({ label, color, desc }) => (
                <div key={label} className="flex items-center gap-3 bg-slate-800/40 border border-slate-700/40 rounded-xl px-3.5 py-2.5">
                  <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full border shrink-0 ${color}`}>{label}</span>
                  <span className="text-slate-400 text-xs">{desc}</span>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {COUNTRIES_PREVIEW.map((c) => (
                <span key={c} className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-slate-300">
                  {c}
                </span>
              ))}
            </div>
          ...
          <div className="border-t border-slate-700/60 pt-2">
            <p className="text-slate-400 text-sm font-semibold text-center">Accéder à votre espace</p>
          </div>
        </div>

        {/* ── FORM (shared on both mobile & desktop right column) ── */}
        <div className="flex flex-col items-center justify-center px-4 sm:px-8 pb-10 lg:flex-1 lg:justify-center lg:py-10">

          {/* Desktop welcome text */}
          <div className="hidden lg:block text-center mb-6 w-full max-w-sm">
          ...
          <div className="w-full max-w-sm animate-scale-in">
            ...
            {/* Mobile trust footer */}
            <div className="lg:hidden mt-5 flex flex-wrap items-center justify-center gap-x-3 gap-y-1.5">
              <div className="flex items-center gap-1.5 text-xs text-slate-500">
            ...
            {/* Footer note */}
            <p className="text-xs text-slate-500 text-center mt-4 leading-relaxed max-w-sm mx-auto px-4 pb-6">
